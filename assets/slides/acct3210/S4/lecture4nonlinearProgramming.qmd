...
2. The lectures are not recorded. _I am recording supplemental versions for the add drop period, if these are useful (and time allows) I will continue to make them._
...
- Guess and check? 
- Set up and solve a Legrangian :) I am not going to ask you to _actually_ solve this by hand... you can do it with Excel and Python!
...
Plug $q_1 = 196.43$ and $K=200$ into: 
$$\Pi(P,C) \equiv 275 \times q_1 - 140 \times \frac{q_1^2}{K} - 100 \times K $$
...
#| echo: true
profit = (
  275*q1.value[0] - 140*((q1.value[0]**2)/k.value[0]) - 100*k.value[0]
)
...
m.solve(disp=False)
profit = (
  300*q2.value[0] - 175*((q2.value[0]**2)/k.value[0]) - 100*k.value[0]
)
...
1. Find the optimal production plan for each combination.
2. Increment and find the optimal production plan for the incremented combinations. 
3.  Take difference to get marginal cost. 
...
<!-- TODO: next year add the derivative calc to the end of the slides -->
