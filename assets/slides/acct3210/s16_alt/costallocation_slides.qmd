...
- In that case we took for granted that we could easily quantify the costs of
making the product.
- We also noted that transferring fixed costs (those that do not vary with
output) is not at all simple... but we didn't do anything about it.
...
## A footnote about fraud

__Fraud is a financial accounting act, but a managerial accounting choice__

- It is the act of _misreporting_ which is illegal.
...
- Gambling for resurrection at FedEx.
   ...
   - Are you paying accountants for high-quality financial statements? Or to 
   write down the numbers that shareholders and regulators want to see? 
...
_We are trying to communicate information about the costs and benefits (synergy) of
coopertative resource allocation within the firm._
...
_The remainder of the semester is focused on this latter question._
...
## What we prefer depends on what information we want to communicate

- Is it better to tax income or floorspace?
- This depends on what the objectives are and what the manager has control over.
- Do we want the manager to maximize income?
- Or do we want the manager to minimize floorspace?
...
| IT                  | 7/12 $\times$ \$6 = \$3.5 | 5/12 $\times$ \$6 = \$2.5 | \$6   |
| Total               |                     \$4.643 |                     \$3.357 | \$8   |
...
While we do not know the correct opportunity cost, we do know that the __direct
allocation method excludes the service departments' use of other service
departments__ and therefore incorrectly states the opportunity cost of each
service department.
...
_In this step we allocate the cost of Telecoms to __IT__, Cars, and Trucks. So
__IT__ includes the cost of Telecoms when we allocate it to Cars and Trucks._ We
use these shares to allocate costs on the next slide.
...
|          | Costs to Allocate |                Telecoms |                 Cars |               Trucks | Total Alloc. |
|----------|------------------:|------------------:|---------------------:|---------------------:|--------------|
...
## Control of overinvestment is the _habit_ of financial accounting systems
...
_Next week we will try to improve on the step-down method._
